// Adapted from rust-by-example/conversion/from_into.md (see ../NOTICES.md).
...
For example we can easily convert a `str` into a `String`. The two-line demo is rendered source-only — it has no `fn main`, so evaluating it would only add `my_str` / `my_string` bindings to evcxr's context for no reader payoff.
...
#evcxr.rust-main(id: "rbe-conversion-from-into-from", ```rust
...
#evcxr.rust-main(id: "rbe-conversion-from-into-into", ```rust
...
#evcxr.rust-main(id: "rbe-conversion-from-into-interchangeable", ```rust
use std::convert::From;
