// Adapted from rust-by-example/flow_control/for.md (see ../NOTICES.md).
...
#evcxr.rust-main(id: "rbe-flow-for-range", ```rust
fn main() {
...
#evcxr.rust-main(id: "rbe-flow-for-range-inclusive", ```rust
fn main() {
    // `n` will take the values: 1, 2, ..., 100 in each iteration
    for n in 1..=100 {
        if n % 15 == 0 {
...
Just remember that even though you can compile the code when a>b, the loop gets never executed. Source-only — runs but prints nothing.
...
If you want to count down, you need to use `.rev()` instead. Source-only here too — bare statements without a `fn main` wrapper.
...
#evcxr.rust-main(id: "rbe-flow-for-iter", ```rust
fn main() {
...
#evcxr.rust-main(id: "rbe-flow-for-into-iter", ```rust
fn main() {
    let names = vec!["Bob", "Frank", "Ferris"];
...
#evcxr.rust-main(id: "rbe-flow-for-iter-mut", ```rust
fn main() {
