// Adapted from rust-by-example/primitives/array.md (see ../NOTICES.md).
...
Upstream tags this snippet `ignore,mdbook-runnable` defensively (the commented-out lines at the bottom *would* panic if uncommented), but the un-commented body uses `xs.get(i)` to handle out-of-bounds gracefully and runs cleanly. Evaluated faithfully through `rust-main(...)` here.

#evcxr.rust-main(id: "rbe-primitives-array", ```rust
use std::mem;
