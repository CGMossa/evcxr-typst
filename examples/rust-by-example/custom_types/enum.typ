// Adapted from rust-by-example/custom_types/enum.md (see ../NOTICES.md).
...
#evcxr.rust-main(id: "rbe-custom-types-enum-webevent", ```rust
...
#evcxr.rust-main(id: "rbe-custom-types-enum-alias", ```rust
...
The most common place you'll see this is in `impl` blocks using the `Self` alias. Rendered as source only — evaluating it would redefine `VeryVerboseEnumOfThingsToDoWithNumbers` against the prior snippet's persistent definition for no payoff, since this block has no `fn main()`.
