// Adapted from rust-by-example/types/cast.md (see ../NOTICES.md).
...
Upstream tags this `ignore,mdbook-runnable` because it contains two deliberate compile errors: `let integer: u8 = decimal;` (no implicit float→int) and `let character = decimal as char;` (no float→char). Rendered source-only to preserve the lesson.
