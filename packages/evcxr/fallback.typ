...
#let placeholder(kind, id, src: none) = block(
  fill: rgb("fffae6"),
  ...
  width: 100%,
  {
    ...
    if src != none {
      // Dimmed source excerpt so authors can identify which snippet is unresolved.
      text(fill: luma(160), raw(src, lang: "rust", block: true))
    }
  },
)
