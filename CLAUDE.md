...
- **Do not run `cargo` (test, build, run, check) through `rtk` in this repo.** rtk filters clean-pass output down to nothing, so a successful `cargo test` looks like a hung command and the human-visible signal is lost. If a hook would wrap your invocation transparently, prefix with `rtk proxy` (e.g. `rtk proxy cargo test --workspace -- --test-threads 1`) to bypass filtering and stream the real cargo output. This rule applies to anything that depends on streamed cargo output — `--test-threads 1` runs especially, since they take 30s+ and "nothing on stdout" is indistinguishable from "still compiling".
