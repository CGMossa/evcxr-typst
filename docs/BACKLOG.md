...
### T-I08 · `min-cli` version enforcement + version policy decision record + artifact version bump

- **Status:** done · `phase4/min-cli-and-version-policy` branch
- **Phase:** 4
- **Depends on:** T-I07 (done)
- **Reference reads:** `docs/DECISIONS.md` D-019 (min-cli mechanism); `docs/design/schema-versioning.md` § 4; `packages/evcxr/lib.typ` `setup()` (confirms `<evcxr-min-cli>` emission); `crates/evcxr-typst/src/discovery.rs` (query plumbing); `crates/evcxr-typst/Cargo.toml`, `packages/evcxr/typst.toml` (version fields)
- **Briefing:** Three deliverables: (A) wire the `<evcxr-min-cli>` marker into the discovery pass and enforce it in `Project::open_with_config` — too-old CLIs return `Error::IncompatibleCliVersion` (exit 2 from the binary); (B) bump both artifacts to `0.1.0` and update `Cargo.lock`; (C) append D-026 (artifact versioning policy) to `docs/DECISIONS.md`.
- **Done when:** `cargo test -p evcxr-typst -- --test-threads 1` clean (new `min_cli_enforcement` integration test + `version_check` unit tests); `cargo clippy`, `fmt --check`, `doc` all clean; `Error::IncompatibleCliVersion` variant public in the library; CLI exits 2 with the prescribed message; D-026 appended; both artifact versions at `0.1.0`.
- **Resolution:** Landed on `phase4/min-cli-and-version-policy`. Discovery now runs a third `typst query` for `<evcxr-min-cli>` (optional, returns empty on no `setup(min-cli:)`), takes the highest of any multiple markers, and returns it in `DiscoveryResult`. `Project::open_with_config` calls `version_check::check` with the result and returns `Err(Error::IncompatibleCliVersion { required, actual })` when the running binary is too old; the CLI maps this to `eprintln!` + `std::process::exit(2)`. The watch loop re-checks on each cycle so live edits that raise `min-cli` are caught immediately. Pure tuple-comparison version checker (`version_check.rs`) covers pre-release tags, leading "v", and two-part versions. Both `crates/evcxr-typst/Cargo.toml` and `packages/evcxr/typst.toml` bumped to `0.1.0`; `Cargo.lock` updated. D-026 (independent semver + `min-cli` coupling) appended to `docs/DECISIONS.md`. `schema-versioning.md` § 1 already had `0.1.0` as the target version; no edit needed. Five tests added: four unit tests in `version_check.rs` and four integration tests in `tests/min_cli_enforcement.rs`.
