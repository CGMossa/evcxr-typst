...
2. **How do we handle multi-`.typ` projects (`#include`, `#import` of local files)?** **See T-D09** (multi-file & multi-document layout). The watch loop needs to interface with: (a) a watched-path set that grows beyond `main.typ` to include imported files; (b) a definition of what triggers a full re-classify vs an `IncludeOnly` no-op (e.g. an imported file that contains no `<evcxr-snippet>` markers); (c) cache scope — whether `.evcxr-typst-cache/` lives at the entry-file's directory or at a project root. Out of scope for v0; the watch loop is structured so adding more watched paths is one line in the `watcher.watch(...)` call.
