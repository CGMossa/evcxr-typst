...
1. **Should we deduplicate sidecar writes when the new bytes equal the old?** A leaf-edit that changes only whitespace re-evaluates the snippet; if the resulting `<id>.txt` is byte-identical to what's there, writing it anyway will fire a `notify` event in `typst watch` and trigger a re-render with no visible change. Reading + comparing first costs an extra fs op but saves a render. Default: write unconditionally, revisit if measured cost is real. *Reconcile with `cache.md` once it lands.*

2. **How do we handle multi-`.typ` projects (`#include`, `#import` of local files)?** Today we watch only `main.typ`. If `main.typ` `#import`s `chapter1.typ`, edits to `chapter1.typ` change the snippet list but our watcher doesn't see them. Options: (a) parse the doc to find imports, watch them too; (b) ask `typst query` to also report imported file paths and watch the union; (c) watch the whole project directory and filter on extension. Likely (b), but it depends on what `typst query` exposes. Out of scope for v0, but the watch loop should be structured so adding more watched paths is one line.
